...
suitablearea_biomass = 2500 ## km2 50 * 50
...
obj = (-1000) * (
    ...
    ( 
        ...
        * (cap_bgec / (xr_ref['A_BGEC'].rolling(lon = rollingwindow_bgec, lat = rollingwindow_bgec, min_periods=1,center=True).sum()))
     )
    +
    ( 
        xr_ref['SI_MSW'].rolling(lon = rollingwindow_msw, lat = rollingwindow_msw, min_periods=1,center=True).sum() 
        * (cap_msw / (xr_ref['A_MSW'].rolling(lon = rollingwindow_msw, lat = rollingwindow_msw, min_periods=1,center=True).sum()))
     )
)
